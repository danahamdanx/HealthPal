...
export const generatePrescriptionPDF = async ({
  ...
  const medicinesHtml = medicines.map((med, i) => `
    <tr>
      <td style="padding: 8px; border: 1px solid #ccc;">${i + 1}</td>
      <td style="padding: 8px; border: 1px solid #ccc;">${med.name}</td>
      <td style="padding: 8px; border: 1px solid #ccc;">${med.dosage || '-'}</td>
      <td style="padding: 8px; border: 1px solid #ccc;">${med.duration || '-'}</td>
      <td style="padding: 8px; border: 1px solid #ccc;">${med.instructions || '-'}</td>
    </tr>
  `).join('');
  ...
  const pdfPath = path.resolve(`reports/prescription-${prescriptionId}.pdf`);
  fs.mkdirSync(path.dirname(pdfPath), { recursive: true });
  await page.pdf({ path: pdfPath, format: 'A4', printBackground: true });
